...
function addCorsHeaders(toAdd) {
    ...
    toAdd["Access-Control-Allow-Headers"] = "Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,X-Its-Sdk";
    return toAdd;
}

function processActionAndReturn(actionConfig, event, context, headers, errorMapping) {
    let action = actionConfig.middlewares && actionConfig.middlewares.length > 0 ?
        middleware.combineMiddlewares(actionConfig.middlewares, actionConfig.action) : 
        actionConfig.action;

    return Promise.resolve(action(event, context)).then(res => {
